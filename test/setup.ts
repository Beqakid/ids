...
export async function ensureMigrations() {
  ...
  try {
    const check = await db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='ids_permission_checks'")
      .first();
  ...
  for (const migration of [MIGRATION_1, MIGRATION_2, MIGRATION_3, MIGRATION_4]) {
