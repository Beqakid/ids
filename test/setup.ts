...
CREATE INDEX IF NOT EXISTS idx_ids_platform_context_requests_user_id ON ids_platform_context_requests (user_id);
CREATE INDEX IF NOT EXISTS idx_ids_platform_context_requests_requester_type ON ids_platform_context_requests (requester_type);
CREATE INDEX IF NOT EXISTS idx_ids_platform_context_requests_requester_client_id ON ids_platform_context_requests (requester_client_id);
CREATE INDEX IF NOT EXISTS idx_ids_platform_context_requests_target_app_id ON ids_platform_context_requests (target_app_id);
CREATE INDEX IF NOT EXISTS idx_ids_platform_context_requests_context_type ON ids_platform_context_requests (context_type);
CREATE INDEX IF NOT EXISTS idx_ids_platform_context_requests_created_at ON ids_platform_context_requests (created_at);
...
CREATE INDEX IF NOT EXISTS idx_ids_kai_action_contexts_user_id ON ids_kai_action_contexts (user_id);
CREATE INDEX IF NOT EXISTS idx_ids_kai_action_contexts_app_id ON ids_kai_action_contexts (app_id);
CREATE INDEX IF NOT EXISTS idx_ids_kai_action_contexts_tenant_id ON ids_kai_action_contexts (tenant_id);
CREATE INDEX IF NOT EXISTS idx_ids_kai_action_contexts_action_key ON ids_kai_action_contexts (action_key);
CREATE INDEX IF NOT EXISTS idx_ids_kai_action_contexts_risk_level ON ids_kai_action_contexts (risk_level);
CREATE INDEX IF NOT EXISTS idx_ids_kai_action_contexts_status ON ids_kai_action_contexts (status);
CREATE INDEX IF NOT EXISTS idx_ids_kai_action_contexts_created_at ON ids_kai_action_contexts (created_at);
...
CREATE INDEX IF NOT EXISTS idx_ids_trust_receipt_envelopes_receipt_type ON ids_trust_receipt_envelopes (receipt_type);
CREATE INDEX IF NOT EXISTS idx_ids_trust_receipt_envelopes_source_app_id ON ids_trust_receipt_envelopes (source_app_id);
CREATE INDEX IF NOT EXISTS idx_ids_trust_receipt_envelopes_source_tenant_id ON ids_trust_receipt_envelopes (source_tenant_id);
CREATE INDEX IF NOT EXISTS idx_ids_trust_receipt_envelopes_user_id ON ids_trust_receipt_envelopes (user_id);
CREATE INDEX IF NOT EXISTS idx_ids_trust_receipt_envelopes_action_context_id ON ids_trust_receipt_envelopes (action_context_id);
CREATE INDEX IF NOT EXISTS idx_ids_trust_receipt_envelopes_status ON ids_trust_receipt_envelopes (status);
CREATE INDEX IF NOT EXISTS idx_ids_trust_receipt_envelopes_created_at ON ids_trust_receipt_envelopes (created_at)
`;

export async function ensureMigrations() {
  const db = (env as unknown as Env).IDS_DB;

  // Check if Phase 6 tables already exist (shared DB with isolatedStorage: false)
  // If they do, all migrations have run — only bootstrap the service key.
  try {
    const check = await db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='ids_platform_context_requests'")
      .first();
    if (check) {
      // Phase 6 tables exist — just ensure the test service key is ready.
  ...
  // ── Phase 6 ───────────────────────────────────────────────────────────────
  for (const sql of MIGRATION_6.split(";")) {
    const trimmed = sql.trim();
    if (trimmed.length > 0) {
      await db.prepare(trimmed).run();
    }
